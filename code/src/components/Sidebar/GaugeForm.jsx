...
class GaugeForm extends React.Component{
    ...
    onSubmit() {
        if(this.isURL(this.state.link)) {
            var values = [this.state.type, this.state.name, this.state.link, this.state.topic, this.state.minVal, this.state.maxVal, this.state.auth, this.state.user, this.state.password];
        ...
        return (
            <div>
                <Modal isOpen={this.state.isActivate} closeRequest={this.closeRequest} className={this.props.className}>
                    <ModalHeader value={this.state.name} toggle={this.closeRequest}>Gauge Chart Settings</ModalHeader>
                    <ModalBody>
                        <form>
                            <FormGroup row>
